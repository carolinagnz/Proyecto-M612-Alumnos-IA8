/**
 * COMPONENTE ALUMNO (MODIFICADO)
 * ================================
 * Muestra la información de un alumno en formato de tarjeta.
 * Ahora incluye botones de editar y eliminar (solo para admins).
 * 
 * Props:
 * - nombre: Nombre del alumno
 * - apellidos: Apellidos del alumno
 * - promo: Promoción del alumno
 * - children: Contenido hijo (Avatar)
 * - esAdmin: Boolean que indica si el usuario es admin
 * - onEdit: Función que se ejecuta al hacer clic en editar
 * - onDelete: Función que se ejecuta al hacer clic en eliminar
 */

function Alumno({ nombre, apellidos, promo, children, esAdmin, onEdit, onDelete }) {
  
  return (
    <div className="bg-white rounded-lg shadow-md p-6 flex items-center gap-4 hover:shadow-xl transition-shadow relative">
      
      {/* Avatar (children) */}
      <div className="flex-shrink-0">
        {children}
      </div>
      
      {/* Información del alumno */}
      <div className="flex-grow">
        <h3 className="text-xl font-bold text-gray-800">
          {nombre} {apellidos}
        </h3>
        <p className="text-sm text-gray-600 mt-1">
          Promoción: {promo}
        </p>
      </div>
      
      {/* Botones de acción (solo visibles para admin) */}
      {esAdmin && (
        <div className="flex gap-2 absolute top-4 right-4">
          
          {/* Botón Editar */}
          <button
            onClick={onEdit}
            className="bg-yellow-500 text-white p-2 rounded-lg hover:bg-yellow-600 transition-colors"
            title="Editar alumno"
          >
            {/* Icono de lápiz */}
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
              <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
            </svg>
          </button>
          
          {/* Botón Eliminar */}
          <button
            onClick={onDelete}
            className="bg-red-500 text-white p-2 rounded-lg hover:bg-red-600 transition-colors"
            title="Eliminar alumno"
          >
            {/* Icono de papelera */}
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
              <path fillRule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd" />
            </svg>
          </button>
          
        </div>
      )}
      
    </div>
  );
}

export default Alumno;